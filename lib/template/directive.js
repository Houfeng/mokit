...
const DirectiveDefinition = new Class({
  ...
  _extends: DirectiveFactory.prototype,
  ...
  _faultHanlde: function (options) {
    options.type = options.type || DirectiveFactory.TYPE_ATTRIBUTE;
    options.level = options.level || DirectiveFactory.LEVEL_GENERAL;
 ...
 */
function DirectiveFactory(options) {
  //创建 definition 实例
  ...
  const Directive = new Class({
    ...
    execute: options.execute || function (scope) {
      this.scope = scope;
      var newValue = utils.isFunction(this.expression) ?
        this.expression.execute(scope) : this.attribute.value;
  ...
  Directive.__proto__ = definition;
  return Directive;
};

//挂载指令定义信息类
DirectiveFactory.Definition = DirectiveDefinition;

//指令类型
DirectiveFactory.TYPE_ATTRIBUTE = 'attribute';
DirectiveFactory.TYPE_ELEMENT = 'element';

//指令级别
DirectiveFactory.LEVEL_ATTRIBUTE = -1000;
DirectiveFactory.LEVEL_GENERAL = 0;
DirectiveFactory.LEVEL_STATEMENT = 1000;
